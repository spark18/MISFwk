...
public class UpLinePreviewTransactionServlet extends BaseAjaxServlet {
	...
	public void exec(HttpServletRequest req, HttpServletResponse resp)
		...
		Query query = session
				.createQuery("from PreTransaction transaction where transaction.user.name='"
						...
						+ PreTransaction.Type.UPLINEPREVIEWLOAD.name() + "' order by transaction.timeStamp DESC");

		List upLineTransactionList = query.list();
		transaction.commit();
		session.close();

		TransactionList transactionList = new TransactionList();
		SimpleDateFormat format = new SimpleDateFormat("yyyyƒÍMM‘¬dd»’ HH:mm:ss");
		for (Object liganTransactionResult : upLineTransactionList) {
		...
		if (upLineTransactionList.isEmpty()) {
			transactionList.setTotal("0");
		} else {
			transactionList
					.setTotal(String.valueOf(upLineTransactionList.size()));
		}
