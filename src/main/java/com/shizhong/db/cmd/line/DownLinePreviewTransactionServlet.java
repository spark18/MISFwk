...
public class DownLinePreviewTransactionServlet extends BaseAjaxServlet {
	...
	public void exec(HttpServletRequest req, HttpServletResponse resp)
		...
		Query query = session
				.createQuery("from PreTransaction transaction where transaction.user.name='"
						...
						+ PreTransaction.Type.DOWNLINEPREVIEWLOAD.name() + "' order by transaction.timeStamp DESC");

		List downLineTransactionList = query.list();
		transaction.commit();
		session.close();

		TransactionList transactionList = new TransactionList();
		SimpleDateFormat format = new SimpleDateFormat("yyyyƒÍMM‘¬dd»’ HH:mm:ss");
		for (Object liganTransactionResult : downLineTransactionList) {
			TransactionRow row = new TransactionRow();
			PreTransaction szTransaction = (PreTransaction) liganTransactionResult;
			row.setId(szTransaction.getId());
			String[] cells = new String[] {
					format.format(szTransaction.getTimeStamp()),
					szTransaction.getComments(), szTransaction.getId().toString(), };
			row.setCells(cells);
			transactionList.addRow(row);
		}

		if (downLineTransactionList.isEmpty()) {
			transactionList.setTotal("0");
		} else {
			transactionList
					.setTotal(String.valueOf(downLineTransactionList.size()));
		}
