...
public class LiGanPreviewTransactionServlet extends BaseAjaxServlet {
	...
	public void exec(HttpServletRequest req, HttpServletResponse resp)
		...
		Query query = session
				.createQuery("from PreTransaction transaction where transaction.user.name='"
						...
						+ PreTransaction.Type.LIGANPREVIEWLOAD.name() + "' order by transaction.timeStamp DESC");

		List liGanTransactionList = query.list();
		transaction.commit();
		session.close();

		TransactionList transactionList = new TransactionList();
		SimpleDateFormat format = new SimpleDateFormat("yyyyƒÍMM‘¬dd»’ HH:mm:ss");
		for (Object liganTransactionResult : liGanTransactionList) {
		...
		if (liGanTransactionList.isEmpty()) {
			transactionList.setTotal("0");
		} else {
			transactionList
					.setTotal(String.valueOf(liGanTransactionList.size()));
		}
