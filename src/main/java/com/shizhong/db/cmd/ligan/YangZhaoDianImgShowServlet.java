...
public class YangZhaoDianImgShowServlet extends BaseAjaxServlet {
	...
	public void exec(HttpServletRequest req, HttpServletResponse resp)
		...
		Query query = session
				.createQuery("from " + entity + " yangzhaodian where yangzhaodian.id=" + rownum);
		...
		Displayable yangzhaodian = (Displayable) yangzhaodianList.get(0);
		transaction.commit();
		session.close();

		try {
			InputStream inputStream = yangzhaodian.getImage().getBinaryStream();
