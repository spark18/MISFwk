...
public class TingZiNanHuiImgShowServlet extends BaseAjaxServlet {
	...
	public void exec(HttpServletRequest req, HttpServletResponse resp)
		...
		Query query = session
				.createQuery("from " + entity + " tingzi where tingzi.id=" + rownum);
		...
		Displayable tingzi = (Displayable) tingziList.get(0);
		transaction.commit();
		session.close();

		try {
			InputStream inputStream = tingzi.getImage().getBinaryStream();
