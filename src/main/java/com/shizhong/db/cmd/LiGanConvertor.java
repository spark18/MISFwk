...
public class LiGanConvertor implements InfoConvertor {
	...
	public PreTransaction persist(BufferedReader reader, User curUser) throws IOException {
		...
		transaction.setComments("导入立杆信息 - [" + "操作人： " + curUser.getName() + "]");
		transaction.setType(PreTransaction.Type.LIGANPREVIEWLOAD.name());
		int count = 0;
		while (null != line) {
		    count++;
...
//				System.out.println(line);
    ...
    private void addToReview(String line, PreTransaction tran) {
		...
		LiGanPreview preview = new LiGanPreview();
...
//			e.printStackTrace();
	...
	private void checkHeaders(String[] headers) {
		if (null == headers || headers.length == 0
				|| !"区域".equals(headers[0].trim())
				|| !"铭牌号".equals(headers[1].trim())
				|| !"路名".equals(headers[2].trim())
				|| !"站名".equals(headers[3].trim())
				|| !"地址".equals(headers[4].trim())
				|| !"车向".equals(headers[5].trim())
				|| !"路线".equals(headers[6].trim())
				|| !"挖坑日期".equals(headers[7].trim())
				|| !"开往方向".equals(headers[8].trim())
				|| !"线路下一站".equals(headers[9].trim())
				|| !"完成日期".equals(headers[10].trim())) {
