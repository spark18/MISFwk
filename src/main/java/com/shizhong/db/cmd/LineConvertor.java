...
public class LineConvertor implements InfoConvertor {
	...
	public PreTransaction persist(BufferedReader reader, User curUser)
		...
		transaction.setComments("导入线路 (" + busTickets[0].getLine() + ") - ["
				+ "操作人： " + curUser.getName() + "]");
		transaction.setType(PreTransaction.Type.UPLINEPREVIEWLOAD.name());
	...
	private IBusTicketPreview convert2BusTicketPreview(String line) {
		...
		try {
			normPrice = Double.parseDouble(cells[NORM]);
		...
		Double airPrice = 2.0;
		try {
			airPrice = Double.parseDouble(cells[AIR]);
		} catch (NumberFormatException e) {
			e.printStackTrace();
		}
		...
		String autosale = "y";
		if ("是".equals(cells[NOSALE])) {
			autosale = "y";
		} else {
			autosale = "n";
		}
		preview.setAutoSale(autosale);
		preview.setOpunit(cells[OPUNIT]);
		preview.setLine(cells[LINE]);
		if ("空调车".equals(bustype)) {
			bustype = "air";
		} else {
			bustype = "norm";
	...
	private void addToReview(int index, String line, PreTransaction tran,
		...
		String[] cells = line.split(",");
		UpLinePreview preview = new UpLinePreview();
		preview.setLineindex(index);
		preview.setTransaction(tran);

		preview.setName(cells[NAME].trim());
		preview.setStatus(EntityPreviewStatus.PREVIEW.name());

		if (ALIAS < cells.length) {
			preview.setAlias(cells[ALIAS].trim());
		}

		if (STOPADDR < cells.length) {
			preview.setStopAddress(cells[STOPADDR].trim());
		}

		if (STARTEND < cells.length) {
			preview.setStartend(cells[STARTEND].trim());
		}
