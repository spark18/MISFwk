$(document).ready(function(){
	...
	function ligangridify(transactionId) {
		...
		$(".ligan").flexigrid({
			url: 'downlinepreview?username=' + $('#username').val() + '&transactionId=' + id,
			dataType: 'json',
			colModel : [
			            {display: '\u4e0b\u884c\u7ad9\u540d', name : 'name', width : 250, sortable : true, align: 'left'},
...
//			            					var downlineListPreview = $.parseJSON(json);
//			            					var rows = downlineListPreview.rows;
...
//			            		});
	...
	function liganhistorygridify() {
		$(".ligan-history").flexigrid({
			url: 'downlinepreviewtransaction?username=' + $('#username').val(),
			dataType: 'json',
			type: 'POST',
			onSuccess: function() {
	...
	$('#file_upload').uploadify({
	    ...
	    'script'    : 'uploadify?',
	    'cancelImg' : 'uploadify/cancel.png',
	    'scriptData': {username: $('#username').val(), type: 'downline'},
	    'method'	: 'get',
	    'folder'    : '/uploads',
