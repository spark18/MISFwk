$(document).ready(function(){
	...
	$('.simple_overlay #submit').click(function() {
		$.ajax({
			...
			data: {id: $('.simple_overlay #rownum').val(),
				   ...
				   factory: $('#mies1 #factory').val(),
				   equiptype: $('#mies1 #equiptype').val(),
				   road: $('#mies1 #road').val(),
				   stop: $('#mies1 #stop').val(),
	...
	function ligangridify(transactionId) {
		...
		$(".ligan").flexigrid({
			url: 'tingziviewlist?username=' + $('#username').val(),
			dataType: 'json',
			colModel : [
			            ...
			            {display: '\u51fa\u5382\u5382\u5bb6', name : 'factory', width : 60, sortable : true, align: 'left'},
			            {display: '\u8bbe\u5907\u7c7b\u578b', name : 'equiptype', width : 60, sortable : true, align: 'left'},
						...
						$('.ligan-grid .ftitle').text($('.ligan-grid .ftitle').text() + ' -- \u8bbe\u5907\u603b\u8ba1 : ' + arguments.callee.caller.caller.arguments[0].numberCount);
			            	$('.ligan tr').each(function(){
			            		...
			            		div.find('a.buslinelink').click(function(){
			            			$('#mies3 input[name=tingziid]').val(_this.find('td[abbr="rownum"] div').text());
			            				...
			            				success: function(json) {
			            					...
			            					table.find('tr').each(function(){
			            						...
			            						$('#mies5 .close').click(function(){
			            							$('#mies5').hide();
			            							var tingziid = $('#mies5 input[name=tingziid]').val();
			            							$('.ligan tr').each(function(){
			            								if($(this).find('td[abbr="rownum"] div').text() == tingziid) {
			            			...
			            			$.ajax({
			            				...
			            				success: function(json) {
			            					...
			            					table.find('tr').each(function(){
			            						...
			            						$(this).find('.viewhistory').click(function(){
			            							//
			            							$('#mies5 input[name=tingziid]').val(_this.find('td[abbr="rownum"] div').text());
			            							$('#mies5 input[name=hname]').val('');
			            							$('#mies5 input[name=halias]').val('');
			            							$('#mies5 input[name=hstopaddress]').val('');
			            							$('#mies5 input[name=hstartend]').val('');
			            			...
			            			$.ajax({
				            			...
				            			success: function(json) {
				            				if('FAILURE' != json) {
					            				...
					            				$('#mies6 input[name=tingziid]').val($('#mies3 input[name=tingziid]').val());
					            				$('#mies6 input[name=line]').val($('#mies3 input[name=line]').val());
					            				$('#mies6 .close').unbind('click');
					            				$('#mies6 .close').click(function(){
					            					$('#mies6').hide();
					            					var tingziid = $('#mies6 input[name=tingziid]').val();
					            					$('.ligan tr').each(function(){
					            						if($(this).find('td[abbr="rownum"] div').text() == tingziid) {
					            							$(this).find('td[abbr=line] .buslinelink').click();
					            						}
					            				...
					            				$('.simple_overlay_line #img-btn-norm #img_upload_norm').uploadify({
						            			    ...
						            			    'scriptData': {'tingziid': $('#mies3 input[name=tingziid]').val(), id: $('#mies3 .tabletic input[name=id]').val(),
				            			...
				            			success: function(json) {
				            				if('FAILURE' != json) {
					            				...
					            				$('.simple_overlay_line #img-btn-air #img_upload_air').uploadify({
						            			    ...
						            			    'scriptData': {'tingziid': $('#mies3 input[name=tingziid]').val(), id: $('#mies3 .tableair input[name=id]').val(),
			            		...
			            		div.find('a[rel="#mies1"]').click(function(){
			            			...
			            			$.ajax({
			            				...
			            				success: function(json) {
			            					...
			            					$('#mies1 #factory').val(liganPreview.factory);
			            					$('#mies1 #equiptype').val(liganPreview.equiptype);
			            					...
			            					$('#mies1 #img img').attr('src', 'tingziimgshow?entity=TingZi&username=' + $('#username').val() + '&id=' + $('.simple_overlay #rownum').val());
			            				}
			            			});
			            			...
			            			$('#img_upload').uploadify({
			            			    ...
			            			    'scriptData': {'id': $('.simple_overlay #rownum').val(),
			            			    	username: $('#username').val(), entity: "TingZi"},
			            			    'method'	: 'get',
			            			    'cancelImg' : 'uploadify/cancel.png',
			            			    'folder'    : '/uploads',
			            			    'fileDesc'  : '\u7acb\u6746\u56fe\u7247',
	...
	function liganmodifyhistorygridify(id) {
		$(".ligan-history").flexigrid({
			url: 'tingzimodifyhistory?username=' + $('#username').val() + '&entity=tingzi&id=' + id,
			dataType: 'json',
			type: 'POST',
			colModel : [
            			...
            			$(this).find('a').click(function(){
            				...
            				$.ajax({
	            				...
	            				success: function(json) {
	            					...
	            					$('#mies4 #hfactory').val(liganPreview.factory).removeClass('text-highlight').parent().removeClass('bg-highlight');
	            					$('#mies4 #hequiptype').val(liganPreview.equiptype).removeClass('text-highlight').parent().removeClass('bg-highlight');
	            					...
	            					$('#mies4 #himg img').attr('src', 'tingziimgshow?entity=TingZi&username=' + $('#username').val() + '&id=' + modifiedField.id).removeClass('text-highlight').parent().removeClass('bg-highlight');
	            					var fields = modifiedField.fields;
	            					for(var index = 0; index < fields.length; index++) {
	            						var field = fields[index];
            				...
            				$.ajax({
                				...
                				success: function(json) {
                					...
                					$('#mies4 #factory').val(liganPreview.factory).removeClass('text-highlight').parent().removeClass('bg-highlight');
                					$('#mies4 #equiptype').val(liganPreview.equiptype).removeClass('text-highlight').parent().removeClass('bg-highlight');
                					...
                					$('#mies4 #img img').attr('src', 'tingziimgshow?entity=TingZi&username=' + $('#username').val() + '&id=' + id).removeClass('text-highlight').parent().removeClass('bg-highlight');
                					var fields = modifiedField.fields;
                					for(var index = 0; index < fields.length; index++) {
                						var field = fields[index];
