$(document).ready(function(){
	...
	function ligangridify() {
		$(".ligan").flexigrid({
			url: 'yangzhaodiansearch?username=' + encodeURIComponent($('#username').val())
				+ '&area=' + encodeURIComponent($('input#line-area').val())
				...
				+ '&direction=' + encodeURIComponent($('input#line-direction').val())
				...
				+ '&yangzhaodiantype=' + encodeURIComponent($('select#line-yangzhaodiantype').val()),
			dataType: 'json',
			colModel : [
			...
			buttons:  [{name: '<a href="./downloadyangzhaodian?username=' + $('#username').val() + '&area=' + encodeURIComponent($('input#line-area').val())
				+ '&picnumber=' + encodeURIComponent($('input#line-number').val())
				+ '&road=' + encodeURIComponent($('input#line-road').val()) + '&stop=' + encodeURIComponent($('input#line-stop').val())
				+ '&address=' + encodeURIComponent($('input#line-address').val())
				+ '&direction=' + encodeURIComponent($('input#line-direction').val())
				...
				+ '&stopnum=' + encodeURIComponent($('input#line-stopnum').val())
				+ '&entitynum=' + encodeURIComponent($('input#line-entitynum').val())
				+ '&yangzhaodiantype=' + encodeURIComponent($('select#line-yangzhaodiantype').val()) + '">\u4e0b\u8f7d\u5e7f\u544a\u4ead</a>'}],
			            usepager: true,
			            title: '\u7acb\u6746\u4fe1\u606f\u8868',
			            useRp: true,
			            rp: 50,
			            ...
			            height: 600,
			            onSuccess: function() {
						$('.ligan-search-result h4').text('\u626c\u62db\u70b9\u641c\u7d22\u7ed3\u679c -- \u8bbe\u5907\u603b\u8ba1 : ' + arguments.callee.caller.caller.arguments[0].numberCount);
			            	$.ajax({
			            		url: 'yangzhaodiancount?entity=YangZhaoDian&username=' + $('#username').val() + '&area=' + encodeURIComponent($('input#line-area').val())
			    				+ '&picnumber=' + encodeURIComponent($('input#line-number').val())
			    				+ '&road=' + encodeURIComponent($('input#line-road').val()) + '&stop=' + encodeURIComponent($('input#line-stop').val())
			    				+ '&address=' + encodeURIComponent($('input#line-address').val())
			    				+ '&direction=' + encodeURIComponent($('input#line-direction').val())
			            	...
			            	$('.ligan tr').each(function(){
			            		...
			            		var div = _this.find('td[abbr="area"] div');
			            		var area = div.text();
			            		div.html($('<a href="javascript:void(0)" rel="#mies1">' + area + '</a>'));
			            			...
			            			$.ajax({
			            				...
			            				success: function(json) {
			            					$('#mies1').dialog({width:680, close:function() {
			            						$('.yangzhaodian-search-fieldset .submit').click();
			            					}});
			            					...
			            					$('#mies1 #img img').attr('src', 'yangzhaodianimgshow?entity=YangZhaoDian&username=' + $('#username').val() + '&id=' + $('.simple_overlay #rownum').val());
			            				}
			            			});
...
//	ligangridify();
	...
	function liganhistorygridify(id) {
		$(".ligan-history").flexigrid({
            ...
            height: 600,
            onSuccess: function() {
            	$('#mies3 input[name=yangzhaodianid]').val(id);
            			...
            			$(this).find('a').click(function(){
            				...
            				$.ajax({
	            				...
	            				success: function(json) {
	            					...
	            					$('#mies5 #hyangzhaodiantype').val(liganPreview.yangzhaodianType).removeClass('text-highlight').parent().removeClass('bg-highlight');
            				...
            				$.ajax({
                				...
                				success: function(json) {
                					...
                					$('#mies5 #yangzhaodiantype').val(liganPreview.yangzhaodianType).removeClass('text-highlight').parent().removeClass('bg-highlight');
